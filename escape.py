...
GAME_MAP += [#["Room name", height, width, Top exit?, Right exit?]
...
objects = {
    ...
    2: [images.soil, None, "It's like a desert.  Or should that bedessert?"],
...
def generate_map():
    ...
    # Add midle lines of room map (wall, floor to fill width, wall).
    ...
    if room_data[4]: # If ext at right of this room
    ...
    if current_room <= MAP_SIZE - MAP_WIDTH: # If room is not on the bottom row
    ...
    center_y = int(HEIGHT / 2) # Center of the game window
...
def draw():
    ...
    for y in range(room_height): # Lay down floor tiles, then items on floor.
        for x in range(room_width):
            ...
            # Next line enables shadows to fall on top of objects in game.
